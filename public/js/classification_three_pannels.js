...
class GraphDimensions {
  constructor(svgSelector="#scatter-plot") {
    ...
    this.panelHeightFractions = [0.4, 0.2, 0.2];
    this.panelNames = ["flux", "fwhm", "pixel_shift"];
    this.panelHeights = [];
...
class Panel {
  ...
  plotVarianceBars(data, yKey = 'fwhm', nBins=10) {
    ...
    self.panel.selectAll(".variance-bar")
      .data(bins)
      .enter().append("rect")
      ...
      .attr("height", self.panelHeight) // Height of the LC panel
      //   .attr("fill", (d, i) => `rgba(255, 0, 0, ${normalizedVariances[i] * 3 / 4})`)
...
class Graph {
  ...
  plot() {
    ...
    let lcPanel = new Panel(svg, graphDimensions, data, 'flux', {xAxis, xScale, 'yLabel': 'Flux', plot: false});
